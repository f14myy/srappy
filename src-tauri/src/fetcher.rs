...
use crate::extractor::{
    extract_clean_text, extract_links, host_matches_domain_rule, normalized_registrable_host,
    post_process,
};
...
pub async fn fetch_page(
    ...
    if opts.use_headless_chrome.unwrap_or(false) {
        // если юзер хочет через браузер, идем другим путем
        return fetch_page_headless(url, opts, stop_flag, active_browsers, browser_id_counter)
            .await;
    }
...
pub async fn fetch_page_headless(
    ...
    let html = tokio::task::spawn_blocking(move || -> Result<String, ScrapeError> {
        ...
        if let Ok(mut browsers) = active_browsers.lock() {
            ...
            // запоминаем браузер, чтобы можно было прибить если что
        ...
        let result = (|| -> Result<String, ScrapeError> {
            let browsers = active_browsers
                .lock()
                .map_err(|_| ScrapeError::Other("Lock error".into()))?;
            let browser = browsers
                .get(&id)
                .ok_or_else(|| ScrapeError::Other("Browser was killed".into()))?;
        ...
        // в конце не забываем убраться за собой
        if let Ok(mut browsers) = active_browsers.lock() {
            browsers.remove(&id);
        }
...
pub fn process_html_to_result(
    ...
    let base_host_opt = Url::parse(url)
        .ok()
    ...
    links.retain(|l| {
        if let Ok(l_parsed) = Url::parse(l) {
            if let Some(host) = l_parsed.host_str() {
                if opts.same_domain_only.unwrap_or(false)
                    && !opts.unique_domains_only.unwrap_or(false)
                {
                ...
                if !whitelist.is_empty()
                    && !whitelist.iter().any(|&w| host_matches_domain_rule(host, w))
                {
                    return false;
                }
                if !blacklist.is_empty()
                    && blacklist.iter().any(|&b| host_matches_domain_rule(host, b))
                {
                    return false;
                }
                true
    ...
    if proxies.is_empty() {
        ...
        for p in proxies {
            let proxy = reqwest::Proxy::all(p)
                .map_err(|e| ScrapeError::Network(format!("Invalid proxy '{}': {}", p, e)))?;
            for ua in &uas {
                let client = Client::builder()
                    .user_agent(*ua)
                    .timeout(std::time::Duration::from_secs(15))
