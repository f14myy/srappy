<script lang="ts">
  ...
  const pagesData = $derived(history.slice(0, 15).reverse().map(h => h.pages_scraped));
  const speedData = $derived(history.slice(0, 15).reverse().map(h => h.avg_speed || 0));
  const charsData = $derived(history.slice(0, 15).reverse().map(h => h.total_chars));
...
<div class="modal-root" onkeydown={backdropKeydown} role="presentation">
  ...
  <div class="modal-backdrop" transition:fade={{ duration: 250 }} onclick={onclose} role="presentation"></div>
  ...
  >
      ...
      <div class="header-actions">
        {#if !selectedItem}
          <button class="btn-clear" onclick={handleClear} disabled={history.length === 0}>
            <Trash2 size={14} /> {tx.stats.clearBtn}
          </button>
        {/if}
    ...
    <div class="stats-content">
      ...
      {:else}
        ...
        <HistoryTable {history} {tx} ondelete={handleDelete} onselect={(item) => selectedItem = item} />
      {/if}
    </div>
  </div>
</div>
...
<style>
  .modal-root { position: fixed; inset: 0; z-index: 5000; display: flex; align-items: center; justify-content: center; padding: 2rem; pointer-events: auto; }
  ...
  .stats-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 2.5rem; width: 100%; }
  .title-group { display: flex; align-items: center; gap: 1rem; }
  .title-group h1 { font-size: 2rem; font-weight: 800; margin: 0; letter-spacing: -0.02em; color: var(--text-primary); }
  .accent-icon { color: var(--accent); }
  .header-actions { display: flex; align-items: center; gap: 1rem; }
  
  .btn-clear { background: rgba(239, 68, 68, 0.1); color: #f87171; border: 1px solid rgba(239, 68, 68, 0.2); padding: 0.5rem 1rem; border-radius: 8px; font-size: 0.85rem; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 0.5rem; transition: all 0.2s; }
  .btn-clear:hover:not(:disabled) { background: rgba(239, 68, 68, 0.2); border-color: rgba(239, 68, 68, 0.4); }
  .btn-clear:disabled { opacity: 0.5; cursor: not-allowed; }
  
  .btn-close { background: transparent; border: none; color: var(--text-muted); cursor: pointer; padding: 0.5rem; border-radius: 8px; transition: all 0.2s; }
  .btn-close:hover { background: rgba(128, 128, 128, 0.1); color: var(--text-primary); }
  
  .stats-content { width: 100%; display: flex; flex-direction: column; gap: 2rem; }

  @media (max-width: 1024px) {
    .modal-root { padding: 1rem; }
