<script lang="ts">
  ...
  import { 
    ArrowLeft, Globe, Clock, FileText, Hash, Database, 
    Zap, Layers, CheckCircle2, AlertCircle, Calendar, Info
  } from "lucide-svelte";
  ...
  const avgChars = $derived(item.pages_scraped > 0 ? Math.round(item.total_chars / item.pages_scraped) : 0);
  const avgSize = $derived(item.pages_scraped > 0 ? Math.round(item.total_size_bytes / item.pages_scraped) : 0);
  const dataDensity = $derived(item.total_size_bytes > 0 ? (item.total_chars / item.total_size_bytes).toFixed(2) : "0");
  const successRate = $derived((item.pages_scraped / (item.pages_scraped + (item.pages_failed || 0))) * 100);
...
<div class="detail-view" in:fly={{ x: 20, duration: 400 }} out:fade={{ duration: 200 }}>
  <button class="back-btn" onclick={onback}>
    <ArrowLeft size={16} /> {tx.stats.backToHistory}
  </button>
  ...
  <div class="detail-grid">
    <div class="main-stats">
        ...
        <StatCard title={tx.results.speed} value={`${(item.avg_speed || 0).toFixed(1)} p/s`} icon={Zap} />
    </div>
...
<style>
  ...
  .back-btn { 
    ...
    color: var(--accent); font-size: 0.85rem; font-weight: 600; cursor: pointer; padding: 0.5rem 0;
  ...
  .url-text { color: var(--accent); font-family: ui-monospace, monospace; font-weight: 600; }
  ...
  .metrics-column { display: flex; flex-direction: column; gap: 0.75rem; }
  .section-title { 
    display: flex; align-items: center; gap: 0.5rem; margin: 0 0 0.5rem; 
    font-size: 0.85rem; color: var(--text-primary); font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em;
  }
  ...
  .metric-row strong.warn { color: #f87171; }

  .mode-tag { padding: 0.1rem 0.4rem; border-radius: 4px; background: rgba(255,255,255,0.05); font-size: 0.75rem; }
  .mode-tag.active { background: rgba(74, 222, 128, 0.1); color: #4ade80; }
</style>
