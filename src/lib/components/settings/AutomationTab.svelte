<script lang="ts">
  ...
  const formats: AppPreferences["defaultExportFormat"][] = [
    "txt",
    "json",
    "csv",
    "csv_meta",
...
<div class="tab">
  ...
  <div class="card-group">
      ...
      <div class="action">
        <div class="chips">
          {#each formats as f}
            <button
              class="chip"
              class:active={prefs.defaultExportFormat === f}
              onclick={() => onpatch({ defaultExportFormat: f })}>.{f.toUpperCase()}</button
            >
          {/each}
        </div>
      </div>
    ...
    <div class="card-row vertical">
      ...
      <div class="action full">
        <input
          ...
          oninput={(e) => onpatch({ filenamePattern: (e.target as HTMLInputElement).value })}
        />
      </div>
    </div>
    ...
    <div class="card-row">
      ...
      <div class="action">
        <Toggle
          on={prefs.autoSaveSessions}
          onclick={() => onpatch({ autoSaveSessions: !prefs.autoSaveSessions })}
        />
      </div>
    </div>
  </div>
