...
<div class="tab">
      ...
      <div class="action full">
        <div class="path-row">
          ...
          <button class="btn-primary small" onclick={pickExportFolder}>
            <FolderOpen size={14} /> {tx.pref.chooseFolder}
          </button>
        </div>
      </div>
    </div>
  ...
  <div class="card-group">
    <div class="card-row">
      ...
      <div class="action">
        <button class="btn-secondary" onclick={exportPrefsFile}><Download size={14} /> {tx.global.save}</button>
      </div>
    </div>
    <div class="card-row">
      <div class="info">
      ...
      <div class="action">
        <button class="btn-secondary" onclick={importPrefsFile}><Upload size={14} /> {tx.pref.chooseFolder}</button>
      </div>
    </div>
  </div>
</div>
...
<style>
  ...
  .btn-primary { background: var(--accent); color: var(--accent-text); border: none; border-radius: 8px; font-size: 0.8rem; font-weight: 600; padding: 0.5rem 1rem; cursor: pointer; display: inline-flex; align-items: center; gap: 0.4rem; transition: opacity 0.2s; font-family: inherit; }
  .btn-primary:hover { opacity: 0.9; }
  .btn-primary.small { padding: 0.35rem 0.75rem; font-size: 0.75rem; }
  ...
  .path-row { display: flex; align-items: center; gap: 0.5rem; width: 100%; background: rgba(128, 128, 128, 0.05); border: 1px solid var(--border-color); border-radius: 8px; padding: 0.3rem 0.3rem 0.3rem 0.75rem; }
  .p-text { flex: 1; font-size: 0.8rem; color: var(--text-muted); font-family: ui-monospace, monospace; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
</style>
