<script lang="ts">
  ...
  const allThemes = $derived({
    dark: [...darkThemes, ...(prefs.customThemes || []).filter(t => t.mode === "dark").map(t => t.id)],
    light: [...lightThemes, ...(prefs.customThemes || []).filter(t => t.mode === "light").map(t => t.id)]
  });
      ...
      <div class="theme-grid">
        {#each (themeMode === 'dark' ? allThemes.dark : allThemes.light) as key}
          ...
          {#if typedTh}
            ...
            >
              <div class="preview"><div class="dot" style="background: {typedTh.accent}"></div></div>
  ...
  <div class="card-group">
    <div class="card-row">
      ...
      <div class="action">
        <div class="slider-row">
          <input type="range" class="input-range" min="0" max="1" step="0.1" value={prefs.gridIntensity} oninput={(e) => onpatch({ gridIntensity: Number((e.target as HTMLInputElement).value) })} />
    ...
    <div class="card-row">
      ...
      <div class="action">
        <div class="chips">
          {#each scales as s}
            <button class="chip" class:active={prefs.uiScale === s.v} onclick={() => onpatch({ uiScale: s.v })}>{s.label}</button>
          {/each}
        </div>
      </div>
    </div>
...
<style>
  ...
  .tab h2 { font-size: 1.5rem; font-weight: 800; margin: 0; color: var(--text-primary); letter-spacing: -0.02em; }
  
  .header-row { display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.5rem; }
  
  .btn-secondary { background: rgba(128, 128, 128, 0.05); border: 1px solid var(--border-color); border-radius: 8px; color: var(--text-primary); font-size: 0.8rem; font-weight: 600; padding: 0.5rem 1rem; cursor: pointer; display: inline-flex; align-items: center; gap: 0.4rem; transition: all 0.2s; font-family: inherit; }
  .btn-secondary:hover { background: rgba(128, 128, 128, 0.1); border-color: var(--text-muted); }
  .btn-secondary.small { padding: 0.35rem 0.75rem; font-size: 0.75rem; }
  ...
  .card-row label { font-size: 0.75rem; font-weight: 700; text-transform: uppercase; color: var(--text-muted); letter-spacing: 0.1em; }
  
  .card-content { padding: 1rem 1.25rem; background: rgba(128, 128, 128, 0.04); border-bottom: 1px solid var(--border-color); }
  .card-content:last-child { border-bottom: none; }
  ...
  .slider-row { display: flex; align-items: center; gap: 1rem; width: 200px; }
  .input-range { flex: 1; height: 4px; border-radius: 99px; appearance: none; background: rgba(128, 128, 128, 0.15); outline: none; }
  .input-range::-webkit-slider-thumb { appearance: none; width: 14px; height: 14px; border-radius: 50%; background: var(--text-primary); cursor: pointer; }
  .val { font-size: 0.8rem; font-weight: 600; color: var(--accent); font-variant-numeric: tabular-nums; width: 36px; text-align: right; }

  .theme-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 0.75rem; }
  .theme-card { background: var(--bg); border: 1px solid var(--brd); border-radius: 10px; padding: 0.6rem; display: flex; flex-direction: column; gap: 0.5rem; align-items: center; cursor: pointer; position: relative; transition: all 0.2s cubic-bezier(0.16, 1, 0.3, 1); }
  .theme-card:hover { transform: translateY(-2px); border-color: var(--text-muted); }
  .theme-card.active { border-color: var(--acc); box-shadow: 0 0 0 1px var(--acc); }
  .preview { width: 100%; height: 36px; background: rgba(128, 128, 128, 0.1); border-radius: 6px; display: flex; align-items: center; justify-content: center; }
  .dot { width: 16px; height: 16px; border-radius: 50%; }
  .theme-card span.theme-name { font-size: 0.65rem; font-weight: 700; text-transform: uppercase; color: var(--txt); max-width: 100%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .check { position: absolute; top: -6px; right: -6px; background: var(--acc); color: var(--accent-text); width: 18px; height: 18px; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 4px rgba(128, 128, 128, 0.2); }
